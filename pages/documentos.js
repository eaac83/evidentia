...
export default function Documentos() {
  ...
  useEffect(() => {
    setIsClient(true);
    const stored = typeof window !== 'undefined' ? JSON.parse(localStorage.getItem("formularioDatos")) : null;
    setDatos(stored);
  }, []);

  if (!isClient || !datos) return <div style={{ padding: '40px', color: 'white' }}>Cargando información...</div>;
