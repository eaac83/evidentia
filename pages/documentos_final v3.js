  ...
  return (
    <div style={{ padding: 40, maxWidth: 900, margin: "auto", color: "#f0f0f0" }}>
      ...
      <div style={{ marginTop: 30, textAlign: "center" }}>
        <button
          onClick={() => {
            const blob = new Blob([textos[tipo]], { type: "application/pdf" });
        ...
        <button
          onClick={() => {
            ...
            if (confirmacion) {
              const blob = new Blob([textos[tipo]], { type: "application/vnd.openxmlformats-officedocument.wordprocessingml.document" });
              const url = URL.createObjectURL(blob);
              const link = document.createElement("a");
              link.href = url;
